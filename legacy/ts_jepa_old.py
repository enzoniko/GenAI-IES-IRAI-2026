# pyright: reportMissingImports=false, reportUnknownVariableType=false, reportUnknownMemberType=false, reportUnknownArgumentType=false, reportMissingParameterType=false, reportUnknownParameterType=false, reportUntypedBaseClass=false, reportUnannotatedClassAttribute=false
import torch
...
class TSJEPA(nn.Module):
    def __init__(self, in_channels=4, patch_size=None, d_model=None, nhead=None, num_layers=None, ema_decay=None, dim_feedforward=None):
        super().__init__()
        
        # Pull from centralized config if not explicitly provided
        patch_size = cfg.JEPA_CONFIG['patch_size'] if patch_size is None else patch_size
        d_model = cfg.JEPA_CONFIG['d_model'] if d_model is None else d_model
        nhead = cfg.JEPA_CONFIG['nhead'] if nhead is None else nhead
        num_layers = cfg.JEPA_CONFIG['num_layers'] if num_layers is None else num_layers
        ema_decay = cfg.JEPA_CONFIG['ema_decay'] if ema_decay is None else ema_decay
        dim_feedforward = cfg.JEPA_CONFIG.get('dim_feedforward', 2048) if dim_feedforward is None else dim_feedforward
