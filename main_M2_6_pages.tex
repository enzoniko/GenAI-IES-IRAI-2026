...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{method_diagram.png}}
...
For this proof-of-concept, we scope the evaluation to a single fixed rotational speed ($\Omega\approx16$\,Hz). % TODO: confirm exact speed bin from MaFaulDa filenames.
...
We evaluate the framework on the synthetic dynamical system to isolate its core mechanisms. \textbf{Phase 1 (Component Validation):} The TS-JEPA encoder successfully captures the system's macro-envelope. As shown in Fig.~\ref{fig:synth_evaluation_imbalance}, Decoder~1 tracks the true envelope with high fidelity, while the CVAE accurately replicates the stochastic high-frequency residual, confirming effective macro/micro disentanglement.  \textbf{Phase 2 (Latent SDEdit):} UMAP projections confirm the latent space forms compact, discriminative fault clusters. During guided diffusion (Fig.~\ref{fig:sdedit_synth}), the SDEdit trajectory navigates cleanly from a healthy starting point to the target imbalance cluster. The physics-informed VJP gradient successfully steers the denoising process toward the fault manifold without requiring fault-specific equations.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{assets/synth_evaluation_imbalance.png}}
\caption{Phase 1 validation (Synthetic). Decoder~1 envelope reconstruction (left) and true high-frequency residual (right).}
\label{fig:synth_evaluation_imbalance}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{assets/synth_umap_sdedit_trajectory_imbalance.png}}
...
\textbf{MaFaulDa Rotor-Bearing Results (Phase 1):}
On the real MaFaulDa dataset (16\,Hz, 4 classes: Normal, Imbalance, Vertical Misalignment, Overhang Ball Bearing), the pipeline achieves reconstruction RMSE of 0.026, 0.064, 0.055, and 0.122 (normalized, per class), with CVAE ELBO of 0.007. The phase-1 twin plot (Fig.~\ref{fig:phase1_mafaulda}) shows reconstruction quality for both the synthetic and real systems. Fig.~\ref{fig:pinn_umap} shows the PINN residual feature space (UMAP, PCA-50, Silhouette\,=\,0.079), demonstrating that physics residuals carry class-discriminative structure even without clean clustering. Fig.~\ref{fig:sdedit_mafaulda} shows the SDEdit trajectory visualization: the guided latent state navigates from healthy toward each target fault class in the TS-JEPA latent space.

\textbf{Transfer Classification (TSTR):}
Table~\ref{tab:tstr} reports the Train-Synthetic-Test-Real (TSTR) transfer classification results. All synthetic methods achieve 25.0\,$\pm$\,0.0\,\% accuracy (chance level for 4 classes), while the real-only upper bound achieves 80.0\,$\pm$\,6.1\,\%. This result reveals a critical limitation: the TS-JEPA latent space collapses to a near-zero-variance representation ($\sigma \approx 10^{-6}$) for real signals, while generated z-vectors occupy a broader distribution ($\sigma \approx 0.35$), creating a domain gap that prevents classifier transfer. Importantly, the 80\,\% real-only accuracy confirms that fault-discriminative structure \emph{exists} in the latent space; the challenge is bridging the real-to-synthetic distribution gap.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{assets/fig_phase1_validation.pdf}}
\caption{Phase 1 twin plot: reconstruction quality on synthetic system (left) and MaFaulDa (right). UMAP latent space colored by fault class.}
\label{fig:phase1_mafaulda}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{assets/fig_pinn_umap_mafaulda.pdf}}
\caption{PINN residual feature space (MaFaulDa, PCA-50, UMAP). Silhouette\,=\,0.079 indicates physics residuals carry class-discriminative structure.}
\label{fig:pinn_umap}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.9\columnwidth]{assets/fig_sdedit_trajectory.pdf}}
\caption{SDEdit trajectory (MaFaulDa): guided latent-space path from healthy toward each target fault class.}
\label{fig:sdedit_mafaulda}
\end{figure}
...
\textbf{Architecture Validation:} Phase 1 results confirm that the modular TS-JEPA + Decoder 1 + CVAE pipeline successfully disentangles macro-envelope from high-frequency jitter on both the synthetic system and MaFaulDa. The reconstruction RMSEs (0.026–0.122 normalized) indicate the framework handles a range of fault complexities, with overhang ball bearing presenting the most complex dynamics.

\textbf{TSTR Gap and Latent Space Collapse:} The TSTR experiment reveals a fundamental challenge: TS-JEPA produces near-zero-variance latent representations for real signals ($\sigma\approx10^{-6}$), while the generative models operate in a broader distribution ($\sigma\approx0.35$). This distribution mismatch prevents classifiers trained on synthetic latent features from generalizing to real ones. The 80\% real-only accuracy confirms latent discriminability exists; the core issue is alignment of the real and synthetic latent distributions. Addressing this through distribution matching, fine-tuning with real prototypes, or contrastive alignment is the primary direction for future work.

\textbf{Physics Guidance Feasibility:} The SDEdit loop with VJP-based PINN guidance runs end-to-end without divergence, demonstrating computational feasibility of physics-constrained latent diffusion. The oracle penalty magnitudes evolve during generation (e.g., Class 3 penalty decreases $14.3\mathrm{M}\to5.5\mathrm{M}$), confirming that physics gradients actively steer the denoising trajectory.

\textbf{Responsible AI Implications:} By anchoring synthesis to healthy-system physics, the framework ensures generated fault traces are interpretable and physically grounded—a prerequisite for trust in safety-critical CPS diagnostics. The identified TSTR limitation is itself a responsible disclosure: the architecture is sound but latent alignment is required before deployment.

\textbf{Future Work:} (1) Latent distribution alignment (prototypical normalization or contrastive fine-tuning of TS-JEPA); (2) Variable-speed generalization beyond the 16\,Hz proof-of-concept; (3) Quantitative physical fidelity metrics using known fault equations (feasible on the synthetic system).


\section{Conclusion}
\label{sec:conclusion}
We presented a physics-guided counterfactual fault synthesis framework combining TS-JEPA disentanglement, modular decoders, and SDEdit with VJP-based PINN guidance. The architecture is validated end-to-end on both a controlled synthetic dynamical system and real MaFaulDa rotor-bearing data. Phase 1 achieves consistent reconstruction quality (RMSE 0.026–0.122 normalized). Phase 2 generates physically constrained counterfactual traces. Transfer classification reveals a latent distribution alignment gap as the primary open challenge, providing a clear roadmap for future work toward physically trustworthy synthetic CPS data.
