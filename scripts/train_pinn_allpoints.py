"""Point-wise PINN training on ALL ~217k normal-class points with optional --no_denorm flag."""
...
CHECKPOINT_PATH = ROOT / "results/pinn_allpoints.pth"

PRINT_EVERY = 100
PARAM_NAMES = ("M1", "M2", "M3", "D1", "D2", "D3", "K1", "K2", "E1")
FALLBACK_SAMPLES = 50_000


def load_allpoints_data(batch_size: int) -> tuple[DataLoader, DataLoader, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    print(f"Loaded ALL normal-class points: {total_points} (from reshape {list(X.shape)} → {list(X_flat.shape)})")
...
def load_fallback_subsample(batch_size: int, n_samples: int = FALLBACK_SAMPLES) -> tuple[DataLoader, DataLoader, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    X = torch.load(X_PATH, map_location="cpu", weights_only=True)
    y = torch.load(Y_PATH, map_location="cpu", weights_only=True)
    ...
    idx = torch.randperm(X_flat.shape[0])[:n_samples]
    X_sub = X_flat[idx]
    y_sub = y_flat[idx]
...
def save_checkpoint(
    ...
    checkpoint: dict[str, Any] = {
        ...
        "y_min": y_min.detach().cpu().clone() if y_min is not None else None,
        "y_max": y_max.detach().cpu().clone() if y_max is not None else None,
        "epoch": epoch,
        "val_loss": val_loss,
...
def run_training(
    ...
    """Core training loop. When no_denorm=True, passes None for y_max/y_min to loss function."""
    arch = cast(dict[str, Any], PINN_POINTWISE_ARCH)
    ...
    X_min = X_min.double()
    X_max = X_max.double()
    y_min_arg: torch.Tensor | None = None if no_denorm else y_min.double()
    y_max_arg: torch.Tensor | None = None if no_denorm else y_max.double()

    if no_denorm:
        print("Using --no_denorm: passing None for y_max/y_min to loss function (normalized values used directly)")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for xb, yb in train_loader:
            ...
            loss_components = loss_fn(model, xb, yb, X_max.double(), X_min.double(), y_max_arg, y_min_arg)
            ...
            if not first_batch_logged and torch.cuda.is_available():
                ...
                print(f"[VRAM] After first batch — allocated: {allocated / 1e9:.3f} GB, reserved: {reserved / 1e9:.3f} GB")
        ...
        if avg_val_loss < (best_val_loss - min_delta):
            ...
            epochs_without_improvement = 0
            save_checkpoint(model, X_min, X_max, y_min_arg, y_max_arg, epoch, best_val_loss, avg_train_loss)
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train PINN on all ~217k normal-class points")
    parser.add_argument("--epochs", type=int, default=None, help="Override number of training epochs")
    parser.add_argument(
        "--no_denorm",
        action="store_true",
        default=False,
        help="Pass None for y_max/y_min to loss function (uses pinn.py normalized-value fallback)",
    )
    ...
    print("Training point-wise PINN on ALL normal-class data (no max_samples limit)")
    print(
        "Hyperparameters: "
        f"batch_size={batch_size}, lr={lr:.4f}, epochs={epochs}, "
        f"patience={patience}, init={arch['unmeasured_net_config']['init_method']}, "
        f"dropout={arch['unmeasured_net_config']['dropout_rate']:.3f}, "
        f"no_denorm={args.no_denorm}"
    )

    try:
        train_loader, val_loader, X_min, X_max, y_min, y_max = load_allpoints_data(batch_size=batch_size)
    except torch.cuda.OutOfMemoryError:
        print(f"[OOM] CUDA out of memory loading all points — falling back to {FALLBACK_SAMPLES:,} subsample")
        torch.cuda.empty_cache()
        train_loader, val_loader, X_min, X_max, y_min, y_max = load_fallback_subsample(
            batch_size=batch_size, n_samples=FALLBACK_SAMPLES
        )
